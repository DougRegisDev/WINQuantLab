...
## Trade duration
duration_candles measures the number of candles during which the position
was effectively active.
The calculation uses the same active observation window used by MFE and MAE:
duration_candles = last_active_index - entry_index + 1

For a normal next-open exit, the exit candle is not counted because the
position is closed at that candle's open.

For a forced final-close exit, the final candle is counted because the
position remains active until that candle's close.
This keeps duration consistent with the excursion observation window.
...
The completed analytical trade record contains:
direction
entry_index
entry_price
exit_index
...
When a DatetimeIndex is available, the engine also preserves:
